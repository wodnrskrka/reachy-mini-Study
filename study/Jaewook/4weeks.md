...
# 데몬 실행 (카메라/오디오 하드웨어 체크 제외)
.\.venv\Scripts\python.exe -m reachy_mini.daemon.app.main --sim --deactivate-audio --kinematics-engine AnalyticalKinematics


최종 실행 순서
터미널 1: 데몬 실행 (이미 켜져 있다면 유지)

터미널 2: .\.venv\Scripts\activate 입력 ((.venv) 확인)

터미널 2: python study/Jaewook/4weeks.py 실행